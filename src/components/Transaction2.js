...
const MyTransactions = () => {
  ...
  const handleCompleteTransaction = async () => {
    ...
    try {
      // Extract transaction IDs for batch processing
      const transactionIds = incompleteTransactions.map(transaction => transaction.transaction_id);
  
      // Make a batch request to update all transaction statuses
      await axios.post('http://localhost:8000/api/update_transactions_status/', {
      ...
      console.error('There was an error updating the transactions!', error);
    }
  };
  ...
  return (
    <Container>
      ...
      <Button onClick={() => handleCompleteTransaction(incompleteTransactions)}>Complete</Button>
    </Container>
  );
};
